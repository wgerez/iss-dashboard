		...
		<div class="page-content">
			...
			<div class="row">
				<div class="col-md-12">
					...
					{{ Form::open(array('url'=>'inscripcionmaterias/modificar', 'class'=>'form-horizontal form-row-seperated', 'id'=>'FrmInscripcionMaterias', 'name'=>'FrmInscripcion'))}}
										...
										<select name="planes" id="planEstudio" class="table-group-action-input form-control">
											...
											@if (isset($planes))
												@foreach ($planes as $plan)
													<option value="{{$plan->id}}"<?php if ($plan->id == $planestudio_id) echo "selected"; ?>>{{$plan->codigoplan}}</option>
												@endforeach
											@endif
										</select>
										<input type="hidden" class="form-control" id="inscripcion_id" name="inscripcion_id" value="{{$inscripcion_id}}">
									</div>
									<!--label class="col-md-2 control-label" for="estadoCiclo">Ciclo lectivo:</label>
									<div class="col-md-4 col-sm-10">
										<label class="control-label {{ $ciclo_act['text'] }}" id="estadoCiclo">{{ $ciclo_act['activo'] }}</label>
									</div-->
								</div>
					...
					<div class="portlet">
						...
						<div class="portlet-body form">
							...
							<div class="form-group <?php if ($errors->has('documento')) echo 'has-error' ?> ">
								...
								<div id="divdni" class="col-md-3">
									...
									<div class="input-icon right">
										...
										<input type="text" id="documento" name="documento" class="form-control" value="{{ $alumno->persona->nrodocumento }}">
							...
							<div class="form-group">
								...
								<div class="col-md-4 col-sm-10">
									<label id="nombreAlumno" class="control-label text-info" id="nombreAlumno">{{ $alumno->persona->apellido }}, {{ $alumno->persona->nombre }}</label>
								</div>
								<label  class="col-md-2 col-sm-2 control-label">DNI:</label>
								<div class="col-md-2 col-sm-4 text-info" id='divDNI'>{{ $alumno->persona->nrodocumento }}</div>
							</div>

							<div class="form-group">
								<label class="control-label col-md-2 col-sm-2">Materias Disponibles:</label>
								<div class="col-md-8 col-sm-8">
									<select multiple="multiple" id="materiasdisponibles" name="materiasdisponibles[]">
										@if (isset($mat_insc))
											@foreach ($mat_insc as $mat_ins)
      											<option value="{{ $mat_ins['id'] }}" <?php if ($mat_ins['selected'] == true) echo "selected"; ?>>{{ $mat_ins['nombre'] }}</option>
											@endforeach
										@endif
									</select>
								</div>
									...
									<tr>
										...
										<tbody>
											@if (isset($mat_insc))
												@foreach ($mat_insc as $mat_ins)
													<?php
													if ($mat_ins['selected'] == true) { ?>
														<tr>
															<td>
																<center>{{ $mat_ins['fecha'] }}</center>
															</td>
															<td>
																<center>{{ $mat_ins['carrera'] }}</center>
															</td>
															<td>
																<center>{{ $mat_ins['anio'] }}°</center>
															</td>
															<td>
																<center>{{ $mat_ins['plan'] }}</center>
															</td>
															<td>
																<center>{{ $mat_ins['nombre'] }}</center>
															</td>
															<!--td>
																<center><a href="#" data-id="{{ $mat_ins['id'] }}" class="btn default btn-xs red btnEliminarMaterias"><i title="Eliminar" class="fa fa-trash-o"></i></a></center>
															</td-->
														</tr>
													<?php
													}
		...
		if (materiasdisponibles) {
			$.each(materiasdisponibles, function(key, value){
				$.ajax({
					...
					data: {'alumnoid': $('#alumnoid').val(), 'planid': $('#planEstudio').val(), 'carrera_id': $('#carreras').val(), 'materiasdisponibles': value},
					type: 'POST'					
		...
		$.ajax({
			...
			if (alumno.apellido!=undefined)	{
				...
				$.ajax({
					...
					data: {'alumnoid': $('#alumnoid').val(), 'plan_id': $('#planEstudio').val(), 'carrera_id': $('#carreras').val()},
					type: 'POST',
					dataType: 'json',
					cache: false					
						...
						if (materias.length > 0) {
							...
							$.each(materias, function(key, value) {
								if (value.selected) {
									...
									$('#table_permisos > tbody:last-child').append('<tr><td><center>'+ value.fecha +'</center></td><td><center>'+ value.carrera +'</center></td><td><center>'+ value.anio +'°</center></td><td><center>'+ value.plan +'</center></td><td><center>'+ value.nombre +'</center></td><!--td><center><a href="#" data-id="'+ value.id +'" class="btn default btn-xs red btnEliminarMaterias"><i title="Eliminar" class="fa fa-trash-o"></i></a></center></td--></tr>');
